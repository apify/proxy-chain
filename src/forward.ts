...
export const forward = async (
    ...
    new Promise(async (resolve, reject) => {
        ...
        const origin =
            proxy?.origin && proxy?.origin !== "null"
                ? proxy.origin
                : request.url;
        ...
        const options: Options = {
            ...
            ...(isSocks && { agent: new SocksProxyAgent(proxy.href) }),
        };
            ...
            try {
                if (proxy.username || proxy.password) {
                    options.headers.push(
                        "proxy-authorization",
                        getBasicAuthorizationHeader(proxy)
                    );
                }
            } catch (error) {
                reject(error);
        ...
        const client = fn(
            origin!,
            options as unknown as http.ClientRequestArgs,
